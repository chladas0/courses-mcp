...
**First time only** - run setup to store your refresh token in the OS keychain:

```sh
courses-mcp --setup
```

Open [courses.fit.cvut.cz](https://courses.fit.cvut.cz) in your browser and log in. Then open **DevTools → Application → Cookies → courses.fit.cvut.cz** - you will see three cookies. Copy the value of **`oauth_refresh_token`** and paste it when prompted.

Setup registers the server in `~/.claude.json` automatically - restart Claude Code afterwards.

The refresh token lasts ~1 month. The server uses it to automatically obtain short-lived access tokens (~2h) in the background - you only need to repeat setup when the refresh token expires.
